...
export async function POST(request: Request) {
  try {
    ...
    const hargaProgram = isBeliAtribut ? program.harga : program.harga - 100000;
    const totalTagihan = hargaProgram + kodeUnik;
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!santri) {
        // Buat Santri Baru, TAPI statusnya dikembalikan ke BARU karena pasti bolong duf'ahnya
        santri = await tx.santri.create({
          data: {
            ...
            kategori: "BARU", // <--- Karena pasti ada dufah yang bolong
