...
export default function MutasiKategoriPage() {
  ...
  useEffect(() => {
    fetch("/api/asrama/mutasi-kategori")
      .then(res => res.json())
      .then(data => {
        if (!data.error) {
      ...
      .catch(err => {
        swalError("Error", "Gagal memuat data mutasi kategori");
      })
    ...
    <Protect permission="view_asrama">
      <div className="p-4 md:p-8 max-w-[1200px] mx-auto min-h-screen">
        ...
        <div className="mb-8 pb-6 border-b border-gold-500/10">
          <h1 className="text-3xl font-extrabold text-gold-500">Mutasi Sakan (Pindah Kategori)</h1>
          <p className="text-gray-400 mt-1 font-medium">
            Daftar santri yang <span className="text-blue-400 font-bold">berubah kategori program</span> (Reguler ↔ Turots) di Duf'ah saat ini dibandingkan dengan Duf'ah sebelumnya.
            Mereka umumnya harus dipindahkan sakannya agar sesuai dengan kategorinya.
        ...
        <div className="bg-dark-800 rounded-2xl shadow-inner border border-gold-500/20 overflow-hidden mb-6">
          <div className="p-4 border-b border-gold-500/10 bg-dark-900/50 flex flex-col md:flex-row gap-4 justify-between items-center">
            ...
            <div className="text-sm font-bold text-gray-400 flex flex-wrap gap-x-4 gap-y-1">
              <span>Total Pindah Kategori: <span className="text-gold-500 font-extrabold">{filteredData.length}</span></span>
              <span>Di Antrean: <span className="text-red-400 font-extrabold">{filteredData.filter((d: any) => d.isAntrean).length}</span></span>
            </div>
          </div>
              ...
              <table className="w-full text-sm text-left border-collapse">
                <thead className="bg-gold-500/10 text-gold-500 uppercase font-black text-xs">
                  <tr>
                    ...
                    <th className="px-4 py-3 border-b border-gold-500/10">Kategori & Sakan Lama</th>
                    <th className="px-4 py-3 border-b border-gold-500/10">Kategori & Posisi Sekarang</th>
                    ...
                    <tr>
                      <td colSpan={7} className="py-12 text-center text-gray-500">
                        <div className="flex flex-col items-center justify-center">
                          <IconHistory />
                          <p className="mt-3 font-medium">Tidak ada santri yang berpindah kategori program saat ini.</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    filteredData.map((item: any, index: number) => (
                      <tr key={index} className={`hover:bg-dark-900/50 transition-colors border-b border-gold-500/5 last:border-0 ${item.isAntrean ? 'bg-red-900/5' : ''}`}>
                        ...
                        <td className="px-4 py-3">
                          <span className="text-xs bg-dark-900 border border-gray-700 px-2 py-0.5 rounded font-bold text-gray-300">{item.programLama}</span>
                          <span className="font-bold text-gray-400 uppercase tracking-widest mt-1 block">{item.sakanLama}</span>
                        </td>
                        <td className="px-4 py-3">
                          <span className="text-xs bg-gold-900/30 border border-gold-500/50 px-2 py-0.5 rounded font-bold text-gold-400">{item.programBaru}</span>
                          <span className={`mt-1 flex font-bold uppercase tracking-widest ${item.sudahDimutasi ? 'text-green-400' : 'text-gray-300'} ${item.isAntrean ? 'text-red-400' : ''}`}>
                             {item.sakanSaatIni}
                          </span>
                          {item.kamarSaatIni !== '-' && (
                        ...
                        <td className="px-4 py-3 text-center">
                          {item.isAntrean ? (
                            <span className="text-[10px] font-black px-2 py-1 rounded bg-red-500/20 text-red-400 border border-red-500/30 animate-pulse">DI ANTREAN</span>
                          ) : item.sudahDimutasi ? (
                            <span className="text-[10px] font-black px-2 py-1 rounded bg-green-500/20 text-green-400 border border-green-500/30">SUDAH PINDAH</span>
                             ...
                             <span className="text-[10px] font-black px-2 py-1 rounded bg-yellow-500/20 text-yellow-400 border border-yellow-500/30">LOKASI SAMA</span>
                          )}
                        </td>
                        <td className="px-4 py-3 text-center">
      ...
      {selectedRiwayat && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-dark-800 rounded-2xl border border-gold-500/30 w-full max-w-lg shadow-2xl p-6 relative flex flex-col">
            ...
            <h2 className="text-xl font-black text-gold-500 mb-1">Riwayat Sakan & Program</h2>
                ...
                return (
                  <div key={i} className={`flex flex-col rounded-xl p-4 shadow-inner relative ${isCurrentMonth ? 'bg-red-900/30 border-2 border-red-500/50' : 'bg-dark-900 border border-gold-500/10'}`}>
                    <span className={`absolute top-0 right-0 text-[10px] font-black px-2 py-1 rounded-bl-xl rounded-tr-xl border-b border-l ${isCurrentMonth ? 'bg-red-500/20 text-red-400 border-red-500/30' : 'bg-gold-500/10 text-gold-500 border-gold-500/10'}`}>
                      {isCurrentMonth ? 'Saat ini' : 'Riwayat'}
                    </span>
                    <span className="text-xs font-bold text-gray-500 mb-2 leading-none">{r.dufah}</span>

                    <span className="text-xs bg-dark-950 border border-gold-500/20 px-2 py-0.5 rounded font-bold text-gold-400 w-fit mb-1">
            ...
            <p className="text-center text-[11px] mt-6 font-bold text-gray-400">
              Riwayat kategori berdasarkan pada program yang dibayar untuk masing-masing pendaftaran Duf'ah.
            </p>
