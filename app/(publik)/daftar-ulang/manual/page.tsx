...
function DaftarUlangManualContent() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/pendaftaran/manual-lama", {
        method: "POST",
    ...
    <div className="min-h-screen bg-dark-900 bg-luxury-pattern text-gray-200 py-10 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl md:text-4xl font-extrabold text-gold-500 mt-4 tracking-wide">Pendaftaran Santri Lama</h1>
          <p className="text-gray-400 mt-2">Khusus Santri Markaz Arabiyah Tanpa NIS (Sebelum Duf'ah 89)</p>
        </div>
                  ...
                  {programs.filter(p => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="text-2xl font-black text-gold-500 mt-2">Rp {new Intl.NumberFormat('id-ID').format(hargaTampil)}</p>
                        {!isBeliAtribut && (
...
export default function DaftarUlangManualPage() {
  return (
